...
func (s *fakeSession) Notify(_ callsession_domain.CallSessionControlMessage) error {
	s.notifies.Add(1)
	return nil
}
...
func TestInProcSessionRegistry_ListPresence(t *testing.T) {
	...
	for _, s := range []callsession_domain.CallSession{aTab1, aTab2, busyWeb, cWeb, cWeb2} {
		if _, err := r.Register(s); err != nil {
			t.Fatal(err)
		}
	}

	byUser := map[string]callsession_domain.MemberPresence{}
	for _, p := range r.ListPresence("ws-1") {
		byUser[p.UserID] = p
	}
	if a := byUser["user-a"]; !a.HasBrowser || a.Busy {
		t.Fatalf("user-a = %+v, want browser, available", a)
	}
	if b := byUser["user-b"]; !b.HasBrowser || !b.Busy {
		t.Fatalf("user-b = %+v, want browser, busy", b)
	}
	if c := byUser["user-c"]; !c.HasBrowser || !c.Busy {
...
func TestInProcSessionRegistry_RejectsInvalid(t *testing.T) {
	r := NewInProcSessionRegistry()
	if _, err := r.Register(nil); !errors.Is(err, ErrNilSession) {
		t.Errorf("nil session: want ErrNilSession, got %v", err)
	}
	if _, err := r.Register(newFakeSession("sid", "uid", "")); !errors.Is(err, callsession_domain.ErrWorkspaceRequired) {
		t.Errorf("empty ws: want ErrWorkspaceRequired, got %v", err)
	}
	if _, err := r.Register(newFakeSession("sid", "", "ws")); !errors.Is(err, callsession_domain.ErrOwnerRequired) {
