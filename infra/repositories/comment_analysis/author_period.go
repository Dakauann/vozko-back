...
//
// The lifetime ranking reads comment_analysis_authors, a projection with one
...
func (r *authorRepository) listAuthorsInPeriod(ctx context.Context, in ca.AuthorsInput) (*shared.PaginatedResult[*ca.AuthorStats], error) {
	...
	err := r.periodQuery(ctx, in).
		...
		Group("comment_analyses.author_external_id, a.id, a.moderation_state, a.role, a.role_confidence, a.role_comments, a.role_rationale").
...
func (r *authorRepository) countAuthorsInPeriod(ctx context.Context, in ca.AuthorsInput) (int64, error) {
	if in.MinComments > 0 {
		...
		err := r.periodQuery(ctx, in).
			Select("COUNT(*) AS n").
			Group("comment_analyses.author_external_id").
			Having(periodHaving(in)).
			Scan(&counted).Error
		if err != nil {
			return 0, err
	...
	var total int64
	err := r.periodQuery(ctx, in).
		Distinct("comment_analyses.author_external_id").
		Count(&total).Error
	return total, err
}
...
func (r *authorRepository) periodQuery(ctx context.Context, in ca.AuthorsInput) *gorm.DB {
	q := r.db.WithContext(ctx).Model(&schema.CommentAnalysis{}).
		Joins(`LEFT JOIN comment_analysis_authors a
			ON a.source = comment_analyses.source
			AND a.account_id = comment_analyses.account_id
			AND a.author_external_id = comment_analyses.author_external_id`).
		Where("comment_analyses.workspace_id = ? AND comment_analyses.deleted_at IS NULL", in.WorkspaceID).
		Where("comment_analyses.author_external_id <> ''")
...
var periodAuthorSelect = `
	comment_analyses.author_external_id AS author_external_id,
	(ARRAY_AGG(comment_analyses.author_handle ORDER BY comment_analyses.commented_at DESC))[1] AS author_handle,
	MIN(comment_analyses.workspace_id::text) AS workspace_id,
	MIN(comment_analyses.source) AS source,
	MIN(comment_analyses.account_id::text) AS account_id,
	MIN(comment_analyses.commented_at) AS first_seen_at,
	MAX(comment_analyses.commented_at) AS last_seen_at,
	a.id AS author_id,
...
func periodAuthorOrderClause(s ca.Sort) string {
	...
	tail := ", a.id ASC, comment_analyses.author_external_id ASC"
	switch s.Key {
	...
	case ca.SortAuthorLastSeen:
		return "MAX(comment_analyses.commented_at) " + dir + tail
	case ca.SortAuthorFirstSeen:
		return "MIN(comment_analyses.commented_at) " + dir + tail
	}
