...
func (r *alertRuleRepository) Update(ctx context.Context, rule *ca.AlertRule) error {
	res := r.db.WithContext(ctx).Model(&schema.CommentAnalysisAlertRule{}).
...
func (r *alertRuleRepository) Delete(ctx context.Context, workspaceID, id string) error {
	res := r.db.WithContext(ctx).
		...
		Delete(&schema.CommentAnalysisAlertRule{})
	if res.Error != nil {
		return res.Error
	}
	if res.RowsAffected == 0 {
...
func (r *alertRuleRepository) FindByID(ctx context.Context, workspaceID, id string) (*ca.AlertRule, error) {
	var row schema.CommentAnalysisAlertRule
...
func (r *alertRuleRepository) ListByAccount(ctx context.Context, workspaceID string, source ca.Source, accountID string) ([]*ca.AlertRule, error) {
	q := r.db.WithContext(ctx).Model(&schema.CommentAnalysisAlertRule{}).
	...
	var rows []schema.CommentAnalysisAlertRule
...
func (r *alertRuleRepository) ClaimFire(ctx context.Context, workspaceID, id string, now time.Time) (bool, error) {
	...
	res := r.db.WithContext(ctx).Exec(`
		UPDATE comment_analysis_alert_rules
		   SET last_fired_at = ?,
...
func (r *alertRuleRepository) RecordFailure(ctx context.Context, workspaceID, id, message string, now time.Time) error {
	...
	return r.db.WithContext(ctx).Model(&schema.CommentAnalysisAlertRule{}).
		Where("workspace_id = ? AND id = ?", workspaceID, id).
...
func alertRulesToDomain(rows []schema.CommentAnalysisAlertRule) []*ca.AlertRule {
...
func alertRuleToDomain(row *schema.CommentAnalysisAlertRule) *ca.AlertRule {
	rule := &ca.AlertRule{
		ID:              row.ID,
...
func alertRuleFromDomain(rule *ca.AlertRule) *schema.CommentAnalysisAlertRule {
	return &schema.CommentAnalysisAlertRule{
		ID:              rule.ID,
