...
func newReservationTestSession() *callSession {
	return newCallSession("sess-1", "user-1", "ws-1", func(*WSOutgoingMessage) {}, nil, nil, 0)
}

func attachTestCall(t *testing.T, s *callSession, callID string) *liveCall {
	t.Helper()
	lc := &liveCall{
...
func TestAttach_FailsWhenAlreadyAttached(t *testing.T) {
	s := newReservationTestSession()
	attachTestCall(t, s, "call-1")
	lc2 := &liveCall{call: newReservationTestCall("call-2"), lifecycleDone: make(chan struct{})}
	if err := s.Attach(lc2); err != errCallSessionBusy {
		t.Fatalf("second Attach err = %v, want errCallSessionBusy", err)
	}
}
