...
type fakeCallSessionCRMCall struct {
	id     string
...
func (c *fakeCallSessionCRMCall) ID() string                            { return c.id }
...
func (c *fakeCallSessionCRMCall) Hangup() error {
...
func (c *fakeCallSessionCRMCall) closeDoneOnce() {
	defer func() { _ = recover() }()
	close(c.done)
}

func (c *fakeCallSessionCRMCall) snapshotAudioCalls() [][]byte {
	c.mu.Lock()
	defer c.mu.Unlock()
...
type fakeEndUseCase struct {
	...
	mu        sync.Mutex
	calls     []callsession_domain.EndOutboundCallInput
}

func (f *fakeEndUseCase) Execute(_ context.Context, in callsession_domain.EndOutboundCallInput) error {
	f.mu.Lock()
...
func (f *fakeEndUseCase) snapshot() []callsession_domain.EndOutboundCallInput {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := make([]callsession_domain.EndOutboundCallInput, len(f.calls))
	copy(out, f.calls)
	return out
}
...
func (r *recordingAdmission) Acquire(_ context.Context, _ callsession_domain.CallAdmissionInput) (*callsession_domain.CallAdmissionLease, error) {
	return nil, nil
}

func (r *recordingAdmission) Refresh(_ *callsession_domain.CallAdmissionLease, _ time.Duration) error {
	return nil
}

func (r *recordingAdmission) Release(_ *callsession_domain.CallAdmissionLease) error {
...
type callSessionTestHarness struct {
	...
	fakeCall  *fakeCallSessionCRMCall
}

func newCallSessionTestHarness(t *testing.T, call *fakeCallSessionCRMCall) *callSessionTestHarness {
	t.Helper()

	adm := &recordingAdmission{}
	// A billing publisher is required at construction now; this test is about
	// admission release, not billing, so it gets a no-op one.
	...
	h := NewCallSessionWSHandler(start, end, lifecycle, allowAllAuthorizer{}, log.Default(), noopWSMetricsRecorder{})
...
func (h *callSessionTestHarness) dial(t *testing.T) *websocket.Conn {
	t.Helper()
...
func (h *callSessionTestHarness) startCall(t *testing.T, c *websocket.Conn) {
	t.Helper()
...
func TestCallSessionWS_HappyPath_ReleasesAdmissionExactlyOnce(t *testing.T) {
	call := newFakeCallSessionCRMCall("crm-happy")
...
func TestCallSessionWS_StuckCall_WSClosed_AdmissionLeaks(t *testing.T) {
	call := newFakeCallSessionCRMCall("crm-stuck")
...
func TestCallSessionWS_RaceBetweenLifecycleExitAndForcedRelease(t *testing.T) {
	call := newFakeCallSessionCRMCall("crm-race")
...
func TestCallSessionWS_CallAudio_ForwardsPCMUnchanged(t *testing.T) {
	call := newFakeCallSessionCRMCall("crm-audio")
	call.closeOnHangup = true
	h := newCallSessionTestHarness(t, call)
...
func TestCallSessionWS_CallAudio_DropsUnsupportedSampleRate(t *testing.T) {
	call := newFakeCallSessionCRMCall("crm-audio-rate")
	call.closeOnHangup = true
	h := newCallSessionTestHarness(t, call)
...
func TestCallSessionWS_CallAudio_ResamplesSupportedRate(t *testing.T) {
	call := newFakeCallSessionCRMCall("crm-audio-resample")
	call.closeOnHangup = true
	h := newCallSessionTestHarness(t, call)
...
// noopBillingPub satisfies the publisher the lifecycle now requires, for tests
