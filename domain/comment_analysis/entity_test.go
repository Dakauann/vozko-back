...
func TestNewPending(t *testing.T) {
	...
	if !a.CommentedAt.Equal(now) {
		t.Errorf("an unknown comment time falls back to now, got %v", a.CommentedAt)
	}
}
...
func TestNewPending_KeepsCommentedAt(t *testing.T) {
	in := newInput("oi")
	posted := now.Add(-90 * 24 * time.Hour)
	in.CommentedAt = posted
	a, err := NewPending(in)
	if err != nil {
		t.Fatal(err)
	}
	if !a.CommentedAt.Equal(posted) {
		t.Fatalf("CommentedAt = %v, want %v", a.CommentedAt, posted)
	}
	if !a.CreatedAt.Equal(now) {
		t.Fatal("CreatedAt is the ingest time and must not follow CommentedAt")
	}
}
...
func TestNewPending_Validation(t *testing.T) {
	cases := map[string]func(*NewInput){
		...
		"comment id required": func(in *NewInput) { in.SourceCommentID = "" },
