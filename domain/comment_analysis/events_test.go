...
func analyzedRow(id string, severity int) *CommentAnalysis {
	at := time.Now().UTC()
...
func TestBatchAnalyzedSkipsUnanalysedRows(t *testing.T) {
	...
	out := NewAnalysisBatchAnalyzed([]*CommentAnalysis{pending, failed, analyzedRow("a", 10), nil})
	if out == nil {
...
func TestBatchAnalyzedIsNilWhenThereIsNothingToShow(t *testing.T) {
	pending := analyzedRow("p", 0)
	pending.Status = StatusPending
	if out := NewAnalysisBatchAnalyzed([]*CommentAnalysis{pending}); out != nil {
...
func TestBatchAnalyzedCapsAndKeepsTheWorst(t *testing.T) {
	rows := make([]*CommentAnalysis, 0, 200)
	for i := 0; i < 200; i++ {
...
func TestBatchAnalyzedReportsNoOverflowWhenItFits(t *testing.T) {
	out := NewAnalysisBatchAnalyzed([]*CommentAnalysis{analyzedRow("a", 5), analyzedRow("b", 90)})
	if out.More != 0 {
