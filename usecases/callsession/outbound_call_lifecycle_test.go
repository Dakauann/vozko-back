...
type fakeAdmission struct {
	...
	releaseCalls int32
	lastReleased *callsession.CallAdmissionLease
	releaseErr   error
}

func (f *fakeAdmission) Acquire(context.Context, callsession.CallAdmissionInput) (*callsession.CallAdmissionLease, error) {
	return f.lease, nil
}
func (f *fakeAdmission) Refresh(*callsession.CallAdmissionLease, time.Duration) error { return nil }
func (f *fakeAdmission) Release(l *callsession.CallAdmissionLease) error {
	atomic.AddInt32(&f.releaseCalls, 1)
	f.mu.Lock()
	defer f.mu.Unlock()
	f.lastReleased = l
...
func TestLifecycleRun_TerminalEventEndsCallAndPublishesBilling(t *testing.T) {
	...
	go func() {
		runner.Run(context.Background(), OutboundCallLifecycleInput{
			...
			WorkspaceID: "ws-1",
			StartedAt:   startedAt,
			Admission:   &callsession.CallAdmissionLease{WorkspaceID: "ws-1", ReservedMicros: 50, PerMinuteCostMicros: 50},
...
func TestLifecycleRun_NilCallWithAdmissionReleasesLease(t *testing.T) {
	...
	lease := &callsession.CallAdmissionLease{WorkspaceID: "ws-1", ReservedMicros: 100, PerMinuteCostMicros: 100}
	done := make(chan struct{})
	go func() {
		runner.Run(context.Background(), OutboundCallLifecycleInput{
			Call:        nil,
...
func TestLifecycleRun_PanicInOnStatusStillReleasesAndPublishes(t *testing.T) {
	...
	go func() {
		...
		runner.Run(context.Background(), OutboundCallLifecycleInput{
			...
			StartedAt:   startedAt,
			Admission:   &callsession.CallAdmissionLease{WorkspaceID: "ws-panic-status", ReservedMicros: 100, PerMinuteCostMicros: 100},
			OnStatus: func(conversation.CallEvent) {
