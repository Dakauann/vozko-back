...
func (s *stubPricer) PriceWhatsAppCategory(string, string) (workspace_pricing.PriceResult, error) {
	return workspace_pricing.PriceResult{}, nil
}
func TestCallAdmissionCoordinatorAcquireSuccess(t *testing.T) {
	...
	lease, err := coordinator.Acquire(context.Background(), callsession.CallAdmissionInput{WorkspaceID: "ws-1"})
	if err != nil {
...
func TestCallAdmissionCoordinatorAcquireInsufficientBalanceReleasesSlot(t *testing.T) {
	...
	lease, err := coordinator.Acquire(context.Background(), callsession.CallAdmissionInput{WorkspaceID: "ws-1"})
	if !errors.Is(err, callsession.ErrInsufficientBalance) {
...
func TestCallAdmissionCoordinatorAcquireTimeout(t *testing.T) {
	...
	lease, err := coordinator.Acquire(context.Background(), callsession.CallAdmissionInput{
		WorkspaceID:      "ws-1",
		SlotPollInterval: 5 * time.Millisecond,
		SlotPollTimeout:  12 * time.Millisecond,
	})
	if !errors.Is(err, callsession.ErrNoCallSlotsAvailable) {
...
var _ callsession.CallSlotGate = (*testSlotGate)(nil)
...
func TestCallAdmissionCoordinatorAcquireEmptyWorkspace(t *testing.T) {
	...
	lease, err := coordinator.Acquire(context.Background(), callsession.CallAdmissionInput{})
	if !errors.Is(err, callsession.ErrWorkspaceRequired) {
...
func TestCallAdmissionCoordinatorAcquirePricerError(t *testing.T) {
	...
	lease, err := coordinator.Acquire(context.Background(), callsession.CallAdmissionInput{WorkspaceID: "ws-1"})
	if !errors.Is(err, callsession.ErrTelephonyPricingUnavailable) {
...
func TestCallAdmissionCoordinatorAcquireContextCancelled(t *testing.T) {
	...
	lease, err := coordinator.Acquire(ctx, callsession.CallAdmissionInput{
		WorkspaceID:      "ws-1",
		SlotPollInterval: 2 * time.Millisecond,
...
func TestCallAdmissionCoordinatorAcquireBalanceCheckError(t *testing.T) {
	...
	lease, err := coordinator.Acquire(context.Background(), callsession.CallAdmissionInput{WorkspaceID: "ws-1"})
	if !errors.Is(err, callsession.ErrBalanceCheckFailed) {
...
func TestCallAdmissionCoordinatorAcquireReserveError(t *testing.T) {
	...
	lease, err := coordinator.Acquire(context.Background(), callsession.CallAdmissionInput{WorkspaceID: "ws-1"})
	if !errors.Is(err, callsession.ErrReservationFailed) {
...
func TestCallAdmissionCoordinatorRefreshNoop(t *testing.T) {
	...
	cases := []*callsession.CallAdmissionLease{
		nil,
		{WorkspaceID: ""},
...
func TestCallAdmissionCoordinatorRefreshError(t *testing.T) {
	...
	lease := &callsession.CallAdmissionLease{WorkspaceID: "ws-1", ReservedMicros: 1_000}
