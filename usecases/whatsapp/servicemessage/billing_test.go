...
// billedReceipt is what Meta sends for a service message it charged for.
...
// The behaviour the whole design turns on: a plan that does not price service
// messages never gates a reply, however empty the balance is. An operator on
// such a plan can answer a customer at zero balance, which is the point.
...
// A plan that does price them is held to it.
...
// The gate asks about the price, not the AI floor. A workspace below the floor
// can still afford a message costing a fraction of a cent, and refusing that
// would take the inbox away from somebody who can pay for what they are doing.
...
// Not being able to read what something costs is not permission to give it
// away.
...
// The category asked for is Meta's own, which is what the catalog is keyed on.
// Asking for anything else would price the message on a line nobody set.
...
func TestChargeDeliveredBooksWhatMetaBilled(t *testing.T) {
	...
	// Refusing the debit would not un-send it, only lose the record of the debt.
...
// Meta retries status webhooks, and one message passes through sent, delivered
// and read. Charging per webhook would bill a single reply three times.
...
// Every one of these is a way of charging for something Meta did not.
...
// A plan priced at zero absorbs the cost rather than passing it on. That is the
// decision the zero expresses, and it must not become a debit.
...
// Missing identifiers are not a reason to charge somebody arbitrary.
...
// A failed idempotency read must not become a double charge on the retry that
// follows. Reporting the error lets the caller log it and move on.
...
// The reference must not collide with the other things that share the column:
// a campaign id, a refund, a template send attempt.
...
// fakeChecker is the balance read and only the balance read.
...
// The constructor refuses rather than degrading, because every one of these
// decides money. A billing that quietly becomes "allow everything and charge
// nothing" when a dependency goes missing is a revenue leak no test fails on
// and no log mentions.
...
// ShouldCharge is the cheap question the status webhook asks before paying
// three database reads to resolve a workspace. It must agree exactly with what
// ChargeDelivered would do, or the webhook skips messages it should charge for.
func TestShouldChargeAgreesWithChargeDelivered(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// And the expensive path must reach the same verdict, or the cheap
			// pre-check would be skipping real charges.
			if err := billing.ChargeDelivered("ws-1", receipt, "wamid.ABC"); err != nil {
				t.Fatalf("ChargeDelivered() = %v", err)
			}
