...
func (h *conversationHarness) row(t *testing.T, id string) *ca.CommentAnalysis {
	t.Helper()
...
func (h *conversationHarness) seed(n int, messagesEach int, lastAt time.Time) {
	for i := 1; i <= n; i++ {
		...
		row, _ := ca.NewPending(ca.NewInput{
			WorkspaceID: "ws-1", Container: convRef(), SourceCommentID: id,
...
func TestEngine_ClassifiesConversationsEndToEnd(t *testing.T) {
	...
	if !row.CommentedAt.Equal(lastAt) {
		t.Errorf("row bucketed at %v, want the last message time %v", row.CommentedAt, lastAt)
	}
