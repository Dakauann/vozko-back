...
func analysedAt(id string, severity int, at time.Time) *ca.CommentAnalysis {
	return &ca.CommentAnalysis{
		ID: id, WorkspaceID: "ws-1", Source: ca.SourceInstagram, AccountID: ref().AccountID,
		ContainerID: ref().ContainerID, SourceCommentID: id,
		AuthorExternalID: "ig-9", AuthorHandle: "fulano",
		Status: ca.StatusAnalyzed, Stance: ca.StanceHostile, Severity: severity,
		Excerpt: "vocês são uns ladrões", CommentedAt: at,
	}
}
...
func TestAlertFiresOnASevereComment(t *testing.T) {
	...
	uc.EvaluateBatch(context.Background(), ref(), "ws-1", []*ca.CommentAnalysis{
...
func TestAlertStaysQuietBelowTheThreshold(t *testing.T) {
	...
	uc.EvaluateBatch(context.Background(), ref(), "ws-1", []*ca.CommentAnalysis{analysedAt("c-1", 79, now)})
...
func TestAlertSkipsDisabledRules(t *testing.T) {
	...
	uc.EvaluateBatch(context.Background(), ref(), "ws-1", []*ca.CommentAnalysis{analysedAt("c-1", 100, now)})
	if len(dispatcher.all()) != 0 {
...
func TestAlertIsWorkspaceScoped(t *testing.T) {
	...
	uc.EvaluateBatch(context.Background(), ref(), "ws-2", []*ca.CommentAnalysis{analysedAt("c-1", 100, now)})
	if len(dispatcher.all()) != 0 {
...
func TestAlertOnlyTheClaimWinnerSends(t *testing.T) {
	...
	batch := []*ca.CommentAnalysis{analysedAt("c-1", 92, now)}
...
func TestAlertFailureIsRecordedAndNotRetriedImmediately(t *testing.T) {
	...
	uc.EvaluateBatch(context.Background(), ref(), "ws-1", []*ca.CommentAnalysis{analysedAt("c-1", 92, now)})
...
func TestAlertFiresOnAWindowedCount(t *testing.T) {
	...
	uc.EvaluateBatch(context.Background(), ref(), "ws-1", []*ca.CommentAnalysis{analysedAt("c-1", 10, now)})
...
func TestAlertWindowedRuleStillEvaluatesOnAnOldBatch(t *testing.T) {
	...
	old := now.Add(-AlertFreshness - time.Hour)
	uc.EvaluateBatch(context.Background(), ref(), "ws-1", []*ca.CommentAnalysis{analysedAt("c-old", 100, old)})
