...
func TestEngine_PostOverrideEnablesAndDisables(t *testing.T) {
	...
	if err := ingest.Enqueue(context.Background(), ca.IngestInput{WorkspaceID: "ws-1", Container: ref(), SourceCommentID: "c-9", AuthorExternalID: "u-9", Text: "ficou ótimo"}); err != nil {
		t.Fatal(err)
	}
	...
	other := ref()
	other.ContainerID = "media-2"
	if err := ingest.Enqueue(context.Background(), ca.IngestInput{WorkspaceID: "ws-1", Container: other, SourceCommentID: "c-10", AuthorExternalID: "u", Text: "x"}); err != nil {
		t.Fatal(err)
	}
