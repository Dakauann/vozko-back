...
//     immune already (they count by commented_at, so old rows fall outside the
...
func (e *alertEvaluator) EvaluateBatch(ctx context.Context, ref ca.ContainerRef, workspaceID string, rows []*ca.CommentAnalysis) {
...
func worstFreshComment(rows []*ca.CommentAnalysis, now time.Time) *ca.CommentAnalysis {
	var worst *ca.CommentAnalysis
	for _, row := range rows {
		if row == nil || row.Status != ca.StatusAnalyzed {
			continue
		}
		if now.Sub(row.CommentedAt) > AlertFreshness {
			continue
		}
...
func commentFor(rule *ca.AlertRule, worst *ca.CommentAnalysis) *ca.CommentAnalysis {
	if rule.Metric.IsWindowed() {
		return nil
	}
	return worst
