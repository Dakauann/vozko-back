...
func TestPurgeJob_DeletesInSlicesAndSparesInFlight(t *testing.T) {
	repo := newFakeRepo()
	for i := 0; i < 3; i++ {
		row, _ := ca.NewPending(ca.NewInput{WorkspaceID: "ws-1", Container: ref(), SubjectID: "old-" + itoa(i),
	...
	fresh, _ := ca.NewPending(ca.NewInput{WorkspaceID: "ws-1", Container: ref(), SubjectID: "fresh",
...
func (r *recordingIngestor) Enqueue(_ context.Context, in ca.IngestInput) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if in.SubjectID == r.errOn {
		return errors.New("boom")
	}
	r.seen = append(r.seen, in.SubjectID)
	return nil
}
