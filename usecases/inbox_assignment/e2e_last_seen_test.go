...
// world is a whole assignment stack over fakes: the real AssignmentService, the
// real CandidateResolver, the real ring maths, the real rescue sweep. Only the
// four ports at the edges are faked, so these tests exercise the code that
...
// worldConfigReader adapts the world's single config to the sweep's two reads,
// so a mode change in a test reaches both the assignment path and the sweep.
...
// disconnect records the departure the way the WS hub would: the presence
// interval closes at the moment they went offline.
...
// stall makes the sweep see this entry as an open roulette hand-out that nobody
// has touched, started at the given time.
...
// Scenario 1 — the team goes home.
//
// The whole point of the feature, told as a day: three agents work, they all
// log off, conversations keep being distributed among them by recency, and once
// they cross the window the inbox stops handing work to people who are not
// coming back today.
func TestE2E_TeamGoesHome(t *testing.T) {
	w := newWorld(t, []string{"ana", "bob", "cid"}, wsc.RouletteModeLastSeen)
	w.connect("ana", "bob", "cid")

	// While everyone is connected, distribution is even — as it always was.
	...
	// Nobody is online, and conversations keep flowing to the same three.
	// This is what the online-only mode could not do: they would have gone
	// unassigned.
	...
	// Three days later they are all past the window.
...
// Scenario 2 — the rescue chain, end to end, with the ownership history it
// leaves behind.
func TestE2E_RescueChainToExhaustion(t *testing.T) {
	...
	// The ring has been walked: unassign so anybody can pick it up.
	w.advance(20 * time.Minute)
	w.stall("stalled", handout)
	w.rescue.hops = 3
	w.sweep()
	...
	assert.Equal(t,
		...
		triggers)
	// Five closes, four appends: every ownership change closed the previous
	// interval, and the final unassignment closed one without opening another.
	// An owner-less open interval would read as "still assigned" in every
	// ownership report.
	assert.Equal(t, 5, w.history.closed)
	assert.Len(t, w.history.appended, 4)
}

// Scenario 3 — the owner opens it in time, so nothing moves.
...
// Scenario 4 — the admin changes their mind mid-flight.
func TestE2E_ModeFlipStopsPendingRescues(t *testing.T) {
	...
	// The admin switches back to the online mode.
	w.cfg.cfg.RouletteMode = wsc.RouletteModeOnline

	// The pending rescue stops on the next tick, with no cleanup pass.
	w.advance(30 * time.Minute)
	w.stall("in-flight", handout)
	w.sweep()
	assert.Equal(t, "ana", w.ownerOf("in-flight"))

	// And the next inbound conversation uses the connected pool again.
	assert.Equal(t, "", w.inbound("after-flip"), "nobody is connected in online mode")
	w.connect("cid")
	assert.Equal(t, "cid", w.inbound("after-flip-2"))
}

// Scenario 5 — the presence pipeline is down.
//
// The telemetry consumer stops, presence ages out, and the workspace must keep
// distributing rather than going quiet workspace-wide.
func TestE2E_PresenceOutageFallsBackToTheOnlinePool(t *testing.T) {
	...
	// The consumer dies: no new presence rows, and the existing ones age out.
	w.advance(72 * time.Hour)
	// Somebody is at their desk with a live socket, but presence never recorded
	// it.
...
// Scenario 6 — the same story on every channel.
//
// There is one code path, so this asserts the property directly: identical
// inputs on four entry types produce identical owners.
...
// The default mode is the historical behaviour, end to end: the roster and
// presence readers are wired, and they are still not consulted.
